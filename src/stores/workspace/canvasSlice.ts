...
export const createCanvasSlice: StateCreator<CanvasSlice> = (set, get) => ({
  ...
  renameCanvas: (canvasId, newName) => {
    set((state) => {
      const updatedCanvases = state.canvases.map(canvas =>
        canvas.id === canvasId ? { ...canvas, name: newName, updatedAt: new Date() } : canvas
      )

      // 更新画布树中的名称
      const updateTreeNames = (nodes: CanvasTreeNode[]): CanvasTreeNode[] => {
        return nodes.map(node => {
          if (node.id === canvasId) {
            return { ...node, name: newName }
          }
          return { ...node, children: updateTreeNames(node.children) }
        })
      }

      return {
        canvases: updatedCanvases,
        canvasTree: updateTreeNames(state.canvasTree),
      }
    })
  },
  ...
  toggleCanvasCollapse: (canvasId) => {
    set((state) => {
      // 更新画布的折叠状态
      const findAndToggle = (nodes: CanvasTreeNode[]): CanvasTreeNode[] => {
        return nodes.map(node => {
          if (node.id === canvasId) {
      ...
      return {
        canvasTree: findAndToggle(state.canvasTree),
      }
    })
  },
  ...
  updateCanvasViewport: (canvasId, viewport) => {
    set((state) => {
      const updatedCanvases = state.canvases.map(canvas =>
        canvas.id === canvasId ? { ...canvas, viewportState: viewport, updatedAt: new Date() } : canvas
      )
