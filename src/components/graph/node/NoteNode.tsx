...
const NoteNode: React.FC<NodeProps<NoteNodeData>> = ({ id, data, selected, ...rest }) => {
  ...
  // 从store中读取展开状态,确保持久化
  const [isExpanded, setIsExpanded] = useState(nodeData?.isExpanded ?? data.isExpanded ?? false);
  ...
  }, [id, getNodeById, nodeData?.isExpanded]);  // 添加nodeData.isExpanded作为依赖
  ...
  }, [isExpanded, isEditing, editContent, editTitle, isEditingTitle, data.title, handleToggleExpand, handleToggleEdit, handleTitleBlur]);
