...
import { ELKLayoutOptions, LayoutOptions, LayoutNodePosition } from '../types/layoutTypes';
import { LAYOUT_CONFIG, PADDING_CONFIG } from '../../../config/layout';
import { ELKConfigBuilder } from './ELKConfigBuilder';
import { logLayoutDebug } from './layoutDebug';
...
export class ELKGraphConverter {
  ...
  static toELKGraph(
    ...
    logLayoutDebug(`ELKGraphConverter: 开始转换 ${nodes.length} 个节点`);

    // 找出所有顶层节点（没有groupId的节点）
    const topLevelNodes = nodes.filter(n => !('groupId' in n) || !n.groupId);

    logLayoutDebug(`找到 ${topLevelNodes.length} 个顶层节点`);

    // 构建ELK根节点
    const elkGraph: ElkNode = {
      id: 'root',
      layoutOptions: this.getDefaultLayoutOptions(options),
      children: this.buildChildren(topLevelNodes, nodes),
      edges: this.convertEdges(edges)
    };

    // 统计信息
    const totalChildren = this.countNodes(elkGraph);
    logLayoutDebug(`转换完成: ${totalChildren} 个节点, ${elkGraph.edges?.length || 0} 条边`);
    ...
    for (const node of currentLevelNodes) {
      ...
      if (node.type === BlockEnum.GROUP) {
        ...
        const children = allNodes.filter(
          currentNode => currentNode.groupId === node.id
        );

        if (children.length > 0) {
          // 递归构建子节点
          elkNode.children = this.buildChildren(children, allNodes);
          ...
          logLayoutDebug(`群组 ${node.id.substring(0, 8)}... 包含 ${children.length} 个子节点`);
        }
      }
   ...
   */
  static fromELKLayout(elkLayout: ElkNode): Map<string, LayoutNodePosition> {
    logLayoutDebug('ELKGraphConverter: 开始提取布局结果');

    const result = new Map<string, LayoutNodePosition>();

    // 递归提取所有节点的位置和尺寸
    this.extractPositions(elkLayout, result, 0, 0);

    logLayoutDebug(`提取完成: ${result.size} 个节点的位置和尺寸`);
  ...
  private static extractPositions(
    elkNode: ElkNode,
    result: Map<string, LayoutNodePosition>,
    parentX: number,
    parentY: number
  ): void {
    // 跳过根节点
    ...
    // ✅ 同时保存位置和尺寸
    const positionData: LayoutNodePosition = {
      x: absoluteX,
      y: absoluteY
    };
    ...
    if (elkNode.children && elkNode.children.length > 0) {
      logLayoutDebug(`递归处理 ${elkNode.children.length} 个子节点...`);
   ...
   */
  private static getDefaultLayoutOptions(options?: LayoutOptions): ELKLayoutOptions {
  ...
  private static getDefaultWidth(node: Node | Group): number {
    ...
    // 普通节点：检查是否展开状态
    const customSize = this.getCustomExpandedSize(node);
    if (customSize?.width) {
  ...
  private static getCustomExpandedSize(node: Node | Group): Node['customExpandedSize'] {
    if (node.type === BlockEnum.NODE && node.isExpanded) {
      return node.customExpandedSize;
    }
